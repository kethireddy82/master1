            ...
            if(state === "SUCCESS"){
                if(response.getReturnValue()!=null && response.getReturnValue()!=undefined){
                    ...
                    if(response.getReturnValue().Assessment__r!=null && response.getReturnValue().Assessment__r!=undefined){
                        console.log('Inside the web verification cmp '+response.getReturnValue().Assessment__r[0].Status__c+' @@'+$A.get("$Label.c.WebVerificationAssessmentStatus"));
                        if(response.getReturnValue().Assessment__r[0].Status__c == $A.get("$Label.c.WebVerificationAssessmentStatus")){
                            component.set('v.showMessage',true);
                            component.set('v.showRatingMessage',false);
                        }
                        else if(response.getReturnValue().Status__c != 'Completed'){
